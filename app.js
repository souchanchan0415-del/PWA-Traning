// Train Punch - responsive UI + body-part categories
const DB_NAME = 'trainpunch_v2';
const DB_VER  = 3; // v3: exercises に by_group インデックス
let db;
...
// ---------- session ----------
function renderPartPills(){
  const wrap = $('#partPills');
  wrap.querySelectorAll('.pill').forEach(p=>{
    p.classList.toggle('active', p.dataset.part === selectedPart);
    p.onclick = async ()=>{
      selectedPart = (selectedPart === p.dataset.part) ? null : p.dataset.part;
      wrap.querySelectorAll('.pill').forEach(x=>x.classList.toggle('active', x===p && selectedPart));
      await renderExSelect();
    };
  });
}
...
function bindSessionUI(){
  $('#btnAddEx').addEventListener('click', async ()=>{
    ...
    if(!name) return;
    try{
      await put('exercises',{name, group: selectedPart || undefined});
  ...
  $('#btnTplApply').addEventListener('click', async ()=>{
    ...
    const pattern = $('#tplPattern').value; // e.g. "5x5"
    const useLastW = $('#tplUseLastW').checked;
    if(!exId || !pattern){ showToast('種目とパターンを選択'); return; }
    const [sets,reps] = pattern.split('x').map(n=>Number(n));
    const weight = useLastW ? await lastWeightForExercise(exId) : 0;
    const date = $('#sessDate').value; const now=Date.now();
    for(let i=0;i<sets;i++){
      currentSession.sets.push({ temp_id: crypto.randomUUID(), exercise_id: exId, weight, reps, rpe:null, ts: now, date });
    }
    renderTodaySets(); showToast('履歴から投入しました');
  });

  // カスタム投入
  $('#btnTplCustom').addEventListener('click', ()=>{
    const sets=Number($('#tplCustomSets').value||0), reps=Number($('#tplCustomReps').value||0), weight=Number($('#tplCustomWeight').value||0);
    const exId = Number($('#tplExCustom').value);
    if(!exId || !sets || !reps){ showToast('種目・セット・回数は必須'); return; }
    const date = $('#sessDate').value; const now=Date.now();
    for(let i=0;i<sets;i++){
      currentSession.sets.push({ temp_id: crypto.randomUUID(), exercise_id: exId, weight, reps, rpe:null, ts: now, date });
    }
...
async function lastWeightForExercise(exId){
  const all = (await getAll('sets')).filter(s=>s.exercise_id===exId).sort((a,b)=>b.ts-a.ts);
  return all[0]?.weight || 0;
}

async function renderExSelect(){
  let exs = await getAll('exercises');
  if (selectedPart) exs = exs.filter(e=>e.group===selectedPart);
  exs.sort((a,b)=> a.name.localeCompare(b.name,'ja'));

  const sel = $('#exSelect');
  sel.innerHTML = exs.map(e=>`<option value="${e.id}">${escapeHTML(e.name)}</option>`).join('');

  // クイック投入/カスタムの種目一覧も同じ候補に
  $('#tplExFromHist').innerHTML = sel.innerHTML;
  $('#tplExCustom').innerHTML   = sel.innerHTML;
...
async function buildPatterns(){
  ...
  const map = new Map(); // (exId -> pattern counts)
  for(const s of sets){
    const key = `${s.exercise_id}|${s.date}`;
    // per date&ex collect reps count; volume-based would need join but keep simple
    if(!map.has(s.exercise_id)) map.set(s.exercise_id, {});
  }
  // pattern from sets per session
  const sessions = await getAll('sessions');
  const byId = Object.fromEntries(sessions.map(s=>[s.id,s]));
  const buckets = {};
  for(const s of sets){
    const sid = s.session_id;
    if(!buckets[s.exercise_id]) buckets[s.exercise_id] = {};
    if(!buckets[s.exercise_id][sid]) buckets[s.exercise_id][sid] = [];
    buckets[s.exercise_id][sid].push(s.reps);
  }
  // count frequency of patterns like "5x5", "3x8"
  const freq = {};
  for(const exId in buckets){
    for(const sid in buckets[exId]){
      const arr = buckets[exId][sid];
      if(!arr.length) continue;
      const reps = arr[0]; // assume uniform
      const pat = `${arr.length}x${reps}`;
      freq[exId] = freq[exId] || {};
      freq[exId][pat] = (freq[exId][pat]||0)+1;
    }
  }
  const exId = Number($('#tplExFromHist').value) || null;
  const box = $('#tplPattern');
  if(!exId || !freq[exId]){ box.innerHTML = '<option value="">パターンなし</option>'; return; }
  const pairs = Object.entries(freq[exId]).sort((a,b)=>b[1]-a[1]);
  box.innerHTML = pairs.map(([k,_])=>`<option value="${k}">${k}</option>`).join('');
}
...
function renderTodaySets(){
  ...
  const exMap = new Map();
  $('#exSelect').querySelectorAll('option').forEach(o=>exMap.set(Number(o.value), o.textContent));
  ul.innerHTML = currentSession.sets.map(s=>{
    const name = exMap.get(s.exercise_id) || '種目';
    return `<li><span><strong>${escapeHTML(name)}</strong> ${s.weight}kg × ${s.reps}${s.rpe?` / RPE${s.rpe}`:''}</span><button data-id="${s.temp_id}" class="ghost">削除</button></li>`;
  }).join('');
  ...
  for (const s of sessions){
    ...
    li.innerHTML = `
      <div>
        ...
        <div style="font-size:12px;color:#9fb0c3">${escapeHTML(s.note || '')}</div>
      </div>
...
async function renderAnalytics(){
  ...
  Object.keys(perEx).forEach(name=>{
    ...
    chip.innerHTML=`<div class="dot" style="background:${color}"></div><span>${escapeHTML(name)}</span>`;
...
async function bindSettingsUI(){
  ...
  $('#btnCreateEx').addEventListener('click', async ()=>{
    ...
    if(!name) return;
    try{ await put('exercises',{name, group: part}); $('#newExName').value=''; await renderExList(); await renderExSelect(); showToast('追加しました'); }
    catch{ showToast('同名の種目があります'); }
  });
...
async function editSessionNote(id){
  ...
  renderHistory(); showToast('メモを更新しました');
}
